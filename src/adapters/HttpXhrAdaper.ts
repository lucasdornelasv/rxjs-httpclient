...
export class HttpXhrAdapter implements HttpAdapter {
	...
	handle<T = any>(req: HttpRequest<T>): Observable<HttpEvent<T>> {
		...
		return new Observable((subscriber: Subscriber<HttpEvent<any>>) => {
			...
			if (!req.headers.has(contentTypeKey)) {
				const detectedType = req.detectContentTypeHeader();
				// Sometimes Content-Type detection fails.
				if (detectedType !== null) {
					xhr.setRequestHeader(contentTypeKey, detectedType);
				}
			}
			...
			if (req.reportProgress) {
				...
				const onDownProgress = (event: ProgressEvent) => {
					...
					if (!sentHeaders) {
						subscriber.next(partialFromXhr());
						sentHeaders = true;
					}
